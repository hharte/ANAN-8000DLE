...
long  SWRHiTime = 0;                                   // timestamp SWR fault
...
void loop() {
  ...
  if ((SWR > 5) and (pow_fwd_disp >> SMULT > 270))  // only when forward power > 270w
  {
    if (SWRHiTime == 0)
    {
      SWRHiTime = millis();   // Found fault - start time
    }
    if ((SWRHiTime > 0) and ((millis() - SWRHiTime) > 1000)) //SWR error and been longer than 1 sec
    {
      FAULT = 6;
      fault();                 // Jump to FAULT handling
    }
  }
  if ((SWRHiTime > 0) and ((millis() - SWRHiTime) > 1000))  // Reset time if non triggeriong event and more than one second elapsed
  {
    SWRHiTime = 0;
  }
