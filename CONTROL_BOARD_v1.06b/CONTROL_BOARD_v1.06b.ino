...
By ON7EQ 02/2015
Modify for the ANAN-8000DLE by W5WC
...
void loop() {  
    ...
    Pratio = (float)pow_fwd_swr / (float)pow_ref_swr;
    SWR = abs ((1+sqrt(Pratio)) / (1-sqrt(Pratio))) ; 
    
    if ((SWR > 2.7) and ((pow_fwd_disp >> SMULT) > 25))   //  only when forward power > 25w
    {
        FAULT = 6;                
        fault();           // Jump to FAULT handling    
    }
    ...
    {   
        ...
        if (((temp >= 0) and (temp <=  5)) or (temp >= 55))     // FAULT check  
        {   
